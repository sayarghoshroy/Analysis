...
print(' — Based on:')
print(' • Two Sample t-test Statistics = '  + str(1 + np.argsort(-1 * np.abs(t_test_stats))))
print(' • Standardized Coefficient = '  + str(1 + np.argsort(-1 * np.abs(std_coefficients))))

"""The same rankings are obtained based on Two Sample t-test Statistics and standardized coefficient values.
...
"""The same individual variable rankings are obtained based on Partial F values and the standardized coefficient values.
...
"""Based on the absolute values of the standardized coefficients, we conclude that y_2, and y_3 contribute significantly to the separation of groups but y_1 and y_4 do not."""
